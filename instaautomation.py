...
Includes keep_alive_ping() which pings MY_RENDER_URL every 10 minutes to prevent host sleeping.
Do NOT remove any commands — all original commands preserved.
"""
...
# -----------------------------
# CONFIG - fill these or set via env vars
# -----------------------------
...
def keep_alive_ping():
    """
    Send ping to Render URL every 10 minutes to prevent sleeping.
    Requires environment variable MY_RENDER_URL set to the public URL.
    """
...
def post_to_instagram(video_path, caption_text):
    try:
        ...
        if client is None:
            ...
            return False
        # instagrapi may require moviepy for video analysis; if missing, catch and report.
...
def main():
    if not BOT_TOKEN or BOT_TOKEN.startswith("YOUR_"):
        print("ERROR: Set BOT_TOKEN (via env var or in script).")
        return

    # Start keep-alive pinger thread (if MY_RENDER_URL provided)
    try:
        t_keep = threading.Thread(target=keep_alive_ping, daemon=True)
        t_keep.start()
        print("Keep-alive pinger thread started (if MY_RENDER_URL provided).")
    except Exception as e:
        print("Failed to start keep-alive thread:", e)
