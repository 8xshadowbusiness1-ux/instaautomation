...
Full & final single-file script with:
 ...
 - Extra safety: handles telegram.error.Conflict by retrying cleanup/startup
"""
import os
import json
import threading
...
from telegram.error import Conflict as TgConflict, TelegramError
...
def receive_video_for_add(update: Update, context: CallbackContext):
    ...
    if msg.video:
        ...
        # maybe user is in schedule flow — let ConversationHandler handle
        return
...
def download_random_from_target_and_add_or_post(post_immediately=True):
    ...
    try:
        ...
        video_medias = []
        for m in medias:
            # media_type 2 often video; instagrapi objects vary by version
            try:
                if getattr(m, "media_type", None) in (2, 8) or getattr(m, "video_url", None):
                    video_medias.append(m)
            except Exception:
                pass
        if not video_medias:
        ...
        try:
            ...
            try:
                ...
                media_url = getattr(info, "video_url", None)
                if not media_url:
                    # try other fields
                    try:
                        for field in ("resources", "thumbnail_url", "display_url"):
                            media_url = getattr(info, field, None)
                            if media_url:
                                break
                    except Exception:
                        pass
                if not media_url:
                    media_url = getattr(chosen, "thumbnail_url", None) or getattr(chosen, "display_url", None)
                if not media_url:
        ...
        try:
            if data.get("autozmode", {}).get("enabled"):
                ...
                with data_lock:
                    ...
                    save_data(data)
                # sleep full interval after run, checking enabled flag regularly
                slept = 0
                while slept < wait:
                    if not data.get("autozmode", {}).get("enabled"):
                        break
                    time.sleep(5)
                    slept += 5
            else:
...
_dispatcher_for_webhook = None
_bot_for_webhook = None
...
# If webhook mode: a route to receive telegram webhook (we forward to dispatcher)
@flask_app.route("/" + (BOT_TOKEN or "token"), methods=["POST"])
def webhook_receiver():
    global _dispatcher_for_webhook, _bot_for_webhook
    if not _dispatcher_for_webhook or not _bot_for_webhook:
        # not ready yet
        return "not ready", 503
    try:
        update_json = request.get_json(force=True)
        update = Update.de_json(update_json, _bot_for_webhook)
        _dispatcher_for_webhook.process_update(update)
        return "ok", 200
    except Exception as e:
        print("Webhook receiver error:", e)
        traceback.print_exc()
        return "error", 500
...
def main():
    ...
    # expose dispatcher & bot to Flask webhook route
    _dispatcher_for_webhook = dp
    _bot_for_webhook = bot
    ...
    # We'll prefer webhook if WEBHOOK_URL provided, else polling.
    # To avoid getUpdates Conflict: delete webhook first, retry if TgConflict occurs.
    started = False
    max_retries = 5
    for attempt in range(1, max_retries + 1):
        try:
            if WEBHOOK_URL:
                ...
                # Set webhook to Flask route
                try:
                    bot.set_webhook(url=full_webhook_url)
                    print("Webhook set to:", full_webhook_url)
                except Exception as e:
                    print("Warning: failed to set webhook, falling back to polling:", e)
                # We still start polling fallback; but if webhook works Telegram will send updates to our Flask route.
                # To be safe, try deleting any existing webhook before polling to avoid getUpdates conflict.
                try:
                    bot.delete_webhook()
                except Exception:
                    pass
                ...
                started = True
                print("Started polling (webhook path also available).")
            else:
            ...
            print(f"Attempt {attempt}: Telegram Conflict error: {e}. Will try to clear webhook and retry.")
            try:
                bot.delete_webhook()
            except Exception as ee:
                print("Failed to delete webhook while handling conflict:", ee)
            time.sleep(2)
        except Exception as e:
            print(f"Attempt {attempt}: Error starting updater: {e}")
            traceback.print_exc()
            time.sleep(2)

    if not started:
        # final attempt to start polling unconditionally
        try:
            print("Final attempt to start polling (last-resort).")
            updater.start_polling()
            started = True
        except Exception as e:
            print("Fatal: couldn't start polling:", e)
            traceback.print_exc()
            return
    ...
    try:
        ...
        print("Updater idle exception:", e)
        traceback.print_exc()
